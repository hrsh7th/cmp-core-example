...
---@alias complete.kit.Vim.Keymap.Keys { keys: string, remap?: boolean }
---@alias complete.kit.Vim.Keymap.KeysSpecifier string|complete.kit.Vim.Keymap.Keys

---@param keys complete.kit.Vim.Keymap.KeysSpecifier
---@return complete.kit.Vim.Keymap.Keys
...
function Keymap.normalize(s)
  local desc = 'complete.kit.Vim.Keymap.normalize'
...
---Send keys.
---@param keys complete.kit.Vim.Keymap.KeysSpecifier|complete.kit.Vim.Keymap.KeysSpecifier[]
...
function Keymap.send(keys, no_insert)
  ...
  return Async.new(function(resolve, _)
    ...
    local callback = Keymap.termcodes(('<Cmd>lua require("complete.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
    if no_insert then
...
function Keymap.to_sendable(callback)
  ...
  return Keymap.termcodes(('<Cmd>lua require("complete.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
end
